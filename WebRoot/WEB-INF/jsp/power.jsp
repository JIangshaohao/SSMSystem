		...
		<div class="tpl-portlet-components">
			...
			<div class="tpl-block">
				<ul class="tpl-task-list">
					<c:forEach items="${course}" var="A" begin="${begin}" end="${end}"
						varStatus="status">
						<li>
							<div class="task-title">
								${status.index + 1} .&nbsp;&nbsp;&nbsp;&nbsp; <span
									class="task-title-sp">${A.mTitle}</span> <span
									class="label label-sm label-warning">${A.mUrl}</span>
								&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<div class="task-config">
								<div class="am-dropdown tpl-task-list-dropdown"
									...
									<ul class="am-dropdown-content tpl-task-list-dropdown-ul">
										<c:if test="${A.rStatus == 1}">
											<li><a href="#" onclick="alert('已启用')"> <i
													class="am-icon-check"></i>启用
											</a></li>
											<li><a href="root_chulipower.action?now=0&rId=${A.rId}">
													<i class="am-icon-pencil"></i>禁用
											</a></li>
										</c:if>
										<c:if test="${A.rStatus == 0}">
											<li><a href="root_chulipower.action?now=1&rId=${A.rId}">
													<i class="am-icon-check"></i>启用
											</a></li>
											<li><a href="#" onclick="alert('已禁用')"> <i
													class="am-icon-pencil"></i>禁用
											</a></li>
										</c:if>
									</ul>
								</div>
						...
						<c:forEach var="x" begin="1" end="${course_allpage}">
							<c:if test="${page == x}">
								<li class="am-active"><a
									href="ssm_paging.action?page=${x}&type=3">${x}</a></li>
							</c:if>
							<c:if test="${page != x}">
