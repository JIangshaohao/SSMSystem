		...
		<div class="tpl-portlet-components">
			...
			<div class="tpl-block">
				<div class="am-g">
							...
							<div class="am-btn-group am-btn-group-xs">
								<button type="button"
									...
									<span class="am-icon-plus"></span> <a
										href="root_insertcourse.action" style="color:white"> 新增 </a>
								</button>
								<button type="button"
									class="am-btn am-btn-default am-btn-secondary">
									<span class="am-icon-save"></span> <a
										href="root_readycourse.action" style="color:white"> 准备状态 </a>
								</button>
								<button type="button"
					...
					<form action="ssm_allcourse.action?sign=0" method="post"
						name="form">
				...
				<div class="am-g">
					<div class="am-u-sm-12">
						<table class="am-table am-table-striped am-table-hover table-main">
							...
							<tbody>
								<c:forEach items="${course}" var="CourseInfo" begin="${begin}"
									end="${end}" varStatus="status">
									<tr>
										<td class="cold">${CourseInfo.coId}</td>
										...
										<td><c:if test="${CourseInfo.coStatus == 0}">
                                            	未选课
                                            </c:if> <c:if
												test="${CourseInfo.coStatus == 1}">
                                            	正在选课	
                                            </c:if> <c:if
												test="${CourseInfo.coStatus == 2}">
                                            	已开始
                                            </c:if> <c:if
												test="${CourseInfo.coStatus == 3}">
                                            	正在进行
                                            </c:if> <c:if
												test="${CourseInfo.coStatus == 4}">
                                            	已结束
                                            </c:if></td>
										<td>
											<div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">
													<button
														...
														<span class="am-icon-pencil-square-o"></span> <a
															href="root_updatecourse.action?index=${status.index}">
															编辑 </a>
													</button>
													<button
														class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only">
														<span class="am-icon-trash-o"></span> <i
															href="root_deletecourse.action?coId=${CourseInfo.coId}"
															class="del"> 删除 </i>
													</button>
												</div>
											</div>
									...
									<c:forEach var="x" begin="1" end="${course_allpage}">
										<c:if test="${page == x}">
											<li class="am-active"><a
												href="ssm_paging.action?page=${x}&type=2">${x}</a></li>
										</c:if>
										<c:if test="${page != x}">
...
<!-- <script type="text/javascript">
	$(function() {
		$(".del").click(function() {
			//alert("进来了");
			var coId = $(this).parents("tr").children("td.cold").text();
			//alert(coId);
			$.ajax({
				url : "/SSMSystem/root_deletecourse.action",
				type : "post",
				data : {
					"coId" : coId
				},
				dataType : "text",
				success : bkbk($(this).parents("tr"))
			});
			function bkbk(tr) {
				//alert("tr" );
				$(tr).remove();
